...
if CLIENT then
    ...
    function HUDELEMENT:DrawClassOption(ty, key, name, color, key_width)
		...
		-- draw key
		draw.AdvancedText(key, "OctagonalRole", x - self.pad - 0.5 * key_width, ty + self.optionHeight * 0.5, self:GetDefaultFontColor(self.basecolor), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, false, self.scale)

		-- draw class name
		draw.AdvancedText(name, "OctagonalMSTACKMsg", x + w * 0.5, ty + self.optionHeight * 0.5, self:GetDefaultFontColor(color), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, false, self.scale)
	end
